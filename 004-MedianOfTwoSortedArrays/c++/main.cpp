...
public:
    ...
    double findMedianSortedArrays4_1(vector<int>& nums1, vector<int>& nums2) {
        ...
        if ((m + n) & 0x01) {
	    if (m == 0)
 	    	return kth_4_1(NULL, m, &*nums2.begin(), n, k);
	    else if (n == 0) 
		return kth_4_1(&*nums1.begin(), m, NULL, n, k);
	    else return kth_4_1(&*nums1.begin(), m, &*nums2.begin(), n, k);
	}
        else {
            int a, b;
            if (m == 0) {
		a = kth_4_1(NULL, m, &*nums2.begin(), n, (m + n) / 2);
		b = kth_4_1(NULL, m, &*nums2.begin(), n, (m + n) / 2 + 1);
	    }
            else if (n == 0) {
		b = kth_4_1(&*nums1.begin(), m, NULL, n, (m + n) / 2 + 1);
		a = kth_4_1(&*nums1.begin(), m, NULL, n, (m + n) / 2);
	    }
            else{
